    ...
    for task_id in tasks:
        ...
        while not done:
            ...
            try:
                ...
                strategic_plan = "Order 0 laptops."
            ...
            try:
                ...
                # DEBUG PRINT: Let's see exactly what the Executor built!
                print(f"   [Day {obs.current_day:02} Executor] JSON Sent: {action.model_dump_json()}")
                
            except Exception as e:
                print(f"   ❌ Executor/JSON Error: {e}")
                action = SupplyChainAction(orders=[])

            # ---------------------------------------------------------
            # STEP THE ENVIRONMENT & LOG DAILY PROGRESS
            # ---------------------------------------------------------
